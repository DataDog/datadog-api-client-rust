...
fn test_v2_list_aws_accounts(world: &mut DatadogWorld, _parameters: &HashMap<String, Value>) {
    let api = world
        ...
        .as_ref()
        .expect("api instance not found");
    let response = match block_on(api.list_aws_accounts_with_http_info()) {
        Ok(response) => response,
        Err(error) => {
            return match error {
                Error::ResponseError(e) => {
...
fn test_v2_delete_aws_account(world: &mut DatadogWorld, _parameters: &HashMap<String, Value>) {
    let api = world
        ...
        .expect("api instance not found");
    let aws_account_id =
        serde_json::from_value(_parameters.get("aws_account_id").unwrap().clone()).unwrap();
    let response = match block_on(api.delete_aws_account_with_http_info(aws_account_id)) {
        Ok(response) => response,
        Err(error) => {
            return match error {
                Error::ResponseError(e) => {
...
fn test_v2_get_aws_account(world: &mut DatadogWorld, _parameters: &HashMap<String, Value>) {
    ...
    let aws_account_id =
        serde_json::from_value(_parameters.get("aws_account_id").unwrap().clone()).unwrap();
    let response = match block_on(api.get_aws_account_with_http_info(aws_account_id)) {
        Ok(response) => response,
        Err(error) => {
            return match error {
                Error::ResponseError(e) => {
...
fn test_v2_update_aws_account(world: &mut DatadogWorld, _parameters: &HashMap<String, Value>) {
    ...
    let aws_account_id =
        serde_json::from_value(_parameters.get("aws_account_id").unwrap().clone()).unwrap();
    let body = serde_json::from_value(_parameters.get("body").unwrap().clone()).unwrap();
    let response = match block_on(api.update_aws_account_with_http_info(aws_account_id, body)) {
        Ok(response) => response,
        Err(error) => {
            return match error {
                Error::ResponseError(e) => {
