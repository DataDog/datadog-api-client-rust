...
pub fn collect_function_calls(world: &mut DatadogWorld) {
    ...
    world.function_mappings.insert(
        "v2.GenerateNewExternalID".into(),
        test_v2_generate_new_external_id,
    );
    world.function_mappings.insert(
        "v2.ListAWSLogServices".into(),
        test_v2_list_aws_log_services,
    );
    world
        .function_mappings
...
fn test_v2_generate_new_external_id(
    world: &mut DatadogWorld,
    _parameters: &HashMap<String, Value>,
) {
    let api = world
        ...
        .as_ref()
        .expect("api instance not found");
    let response = match block_on(api.generate_new_external_id_with_http_info()) {
        Ok(response) => response,
        Err(error) => {
            return match error {
                Error::ResponseError(e) => {
...
fn test_v2_list_aws_log_services(world: &mut DatadogWorld, _parameters: &HashMap<String, Value>) {
    let api = world
        .api_instances
        .v2_api_aws_integration
        .as_ref()
        .expect("api instance not found");
    let response = match block_on(api.list_aws_log_services_with_http_info()) {
        Ok(response) => response,
        Err(error) => {
            return match error {
                Error::ResponseError(e) => {
