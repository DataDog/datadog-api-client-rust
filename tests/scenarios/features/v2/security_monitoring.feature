...
Feature: Security Monitoring
  ...
  Scenario: Create a new inbox rule returns "Bad Request" response
    ...
    And body with value {"data": {"attributes": {"action": {"reason_description": "We want to focus on these items."}, "enabled": true, "name": "Rule 1", "rule": {"issue_type": "vulnerability", "query": "key:val", "rule_ids": ["rule-id-1"], "rule_types": ["application_code_vulnerability"], "severities": ["critical"]}}}}
    When the request is sent
    Then the response status is 400 Bad Request
  ...
  Scenario: Create a new mute rule returns "Bad Request" response
    ...
    And body with value {"data": {"attributes": {"action": {"expire_at": 1893452400000, "reason": "duplicate", "reason_description": "Muting for a while"}, "enabled": true, "name": "Rule 1", "rule": {"issue_type": "vulnerability", "query": "key:val", "rule_ids": ["rule-id-1"], "rule_types": ["application_code_vulnerability"], "severities": ["critical"]}}}}
    When the request is sent
    Then the response status is 400 Bad Request
  ...
  Scenario: Delete a suppression rule returns "Not Found" response
    ...
    And request contains "suppression_id" parameter with value "00000000-0000-0000-0000-426655000000"
    When the request is sent
    Then the response status is 404 Not Found
  ...
  Scenario: Get details of a mute rule returns "Bad Request" response
    ...
    And request contains "mute_rule_id" parameter from "REPLACE.ME"
    When the request is sent
    Then the response status is 400 Bad Request
  ...
  Scenario: Get details of an inbox rule returns "Bad Request" response
    ...
    And request contains "inbox_rule_id" parameter from "REPLACE.ME"
    When the request is sent
    Then the response status is 400 Bad Request
