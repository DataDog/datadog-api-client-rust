...
/// GetIntakeKeyError is a struct for typed errors of method [`IntakeKeyAPI::get_intake_key`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetIntakeKeyError {
...
#[derive(Debug, Clone)]
pub struct IntakeKeyAPI {
    config: datadog::Configuration,
    client: reqwest_middleware::ClientWithMiddleware,
}

impl Default for IntakeKeyAPI {
    fn default() -> Self {
...
impl IntakeKeyAPI {
    ...
    pub async fn get_intake_key(
        &self,
    ) -> Result<crate::datadogV2::model::IntakeAPIKeyResponse, datadog::Error<GetIntakeKeyError>>
    {
        match self.get_intake_key_with_http_info().await {
            Ok(response_content) => {
    ...
    /// Standard Datadog API and application key authentication is not accepted. Authenticate using a
    /// cloud-provider token in the `Authorization: Bearer` header.
    pub async fn get_intake_key_with_http_info(
        &self,
    ) -> Result<
        datadog::ResponseContent<crate::datadogV2::model::IntakeAPIKeyResponse>,
        datadog::Error<GetIntakeKeyError>,
    > {
        let local_configuration = &self.config;
        let operation_id = "v2.get_intake_key";
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            match serde_json::from_str::<crate::datadogV2::model::IntakeAPIKeyResponse>(
                &local_content,
            ) {
                Ok(e) => {
            ...
            let local_entity: Option<GetIntakeKeyError> = serde_json::from_str(&local_content).ok();
