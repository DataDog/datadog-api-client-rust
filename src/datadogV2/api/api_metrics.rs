...
/// GetTagKeyCardinalityDetailsError is a struct for typed errors of method [`MetricsAPI::get_tag_key_cardinality_details`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetTagKeyCardinalityDetailsError {
    APIErrorResponse(crate::datadogV2::model::APIErrorResponse),
    UnknownValue(serde_json::Value),
}
...
impl MetricsAPI {
    ...
    pub async fn get_tag_key_cardinality_details(
        &self,
        metric_name: String,
    ) -> Result<
        crate::datadogV2::model::MetricTagCardinalitiesResponse,
        datadog::Error<GetTagKeyCardinalityDetailsError>,
    > {
        match self
            .get_tag_key_cardinality_details_with_http_info(metric_name)
            .await
        {
            Ok(response_content) => {
                if let Some(e) = response_content.entity {
    ...
    /// Returns the cardinality details of tags for a specific metric.
    pub async fn get_tag_key_cardinality_details_with_http_info(
        &self,
        metric_name: String,
    ) -> Result<
    ...
    > {
        let local_configuration = &self.config;
        let operation_id = "v2.get_tag_key_cardinality_details";
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            ...
            let local_entity: Option<GetTagKeyCardinalityDetailsError> =
                serde_json::from_str(&local_content).ok();
            let local_error = datadog::ResponseContent {
                status: local_status,
                content: local_content,
