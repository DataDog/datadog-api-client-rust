...
/// PatchDORADeploymentByVersionError is a struct for typed errors of method [`DORAMetricsAPI::patch_dora_deployment_by_version`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum PatchDORADeploymentByVersionError {
    JSONAPIErrorResponse(crate::datadogV2::model::JSONAPIErrorResponse),
    APIErrorResponse(crate::datadogV2::model::APIErrorResponse),
    UnknownValue(serde_json::Value),
}
...
impl DORAMetricsAPI {
    ...
    pub async fn patch_dora_deployment_by_version(
        &self,
        body: crate::datadogV2::model::DORADeploymentPatchByVersionRequest,
    ) -> Result<(), datadog::Error<PatchDORADeploymentByVersionError>> {
        match self
            .patch_dora_deployment_by_version_with_http_info(body)
            .await
        {
            Ok(_) => Ok(()),
            Err(err) => Err(err),
    ...
    /// Update a deployment's change failure status, identifying the deployment by its service, environment, and version instead of its ID. Use this to mark a deployment as a change failure or back to stable. You can optionally include remediation details to enable failed deployment recovery time calculation. If multiple deployments match the given service, environment, and version, the most recently finished one is updated.
    pub async fn patch_dora_deployment_by_version_with_http_info(
        &self,
        body: crate::datadogV2::model::DORADeploymentPatchByVersionRequest,
    ) -> Result<datadog::ResponseContent<()>, datadog::Error<PatchDORADeploymentByVersionError>>
    {
        let local_configuration = &self.config;
        let local_operation_id = "v2.patch_dora_deployment_by_version";
        if local_configuration.is_unstable_operation_enabled(local_operation_id) {
            warn!("Using unstable operation {local_operation_id}");
        } else {
            let local_error = datadog::UnstableOperationDisabledError {
                msg: "Operation 'v2.patch_dora_deployment_by_version' is not enabled".to_string(),
            };
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            ...
            let local_entity: Option<PatchDORADeploymentByVersionError> =
                serde_json::from_str(&local_content).ok();
            let local_error = datadog::ResponseContent {
                status: local_status,
                content: local_content,
