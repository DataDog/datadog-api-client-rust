...
/// GetTeamHierarchyLinksOptionalParams is a struct for passing parameters to the method [`TeamsAPI::get_team_hierarchy_links`]
#[non_exhaustive]
#[derive(Clone, Default, Debug)]
pub struct GetTeamHierarchyLinksOptionalParams {
...
impl GetTeamHierarchyLinksOptionalParams {
    /// Specific page number to return.
...
/// GetTeamHierarchyLinksError is a struct for typed errors of method [`TeamsAPI::get_team_hierarchy_links`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetTeamHierarchyLinksError {
    APIErrorResponse(crate::datadogV2::model::APIErrorResponse),
    UnknownValue(serde_json::Value),
}
...
impl TeamsAPI {
    ...
    pub async fn get_team_hierarchy_links(
        &self,
        params: GetTeamHierarchyLinksOptionalParams,
    ) -> Result<
        crate::datadogV2::model::TeamHierarchyLinksResponse,
        datadog::Error<GetTeamHierarchyLinksError>,
    > {
        match self.get_team_hierarchy_links_with_http_info(params).await {
            Ok(response_content) => {
                if let Some(e) = response_content.entity {
                    Ok(e)
                } else {
    ...
    pub fn get_team_hierarchy_links_with_pagination(
        &self,
        mut params: GetTeamHierarchyLinksOptionalParams,
    ) -> impl Stream<
        Item = Result<
    ...
    > + '_ {
        try_stream! {
            ...
            loop {
                let resp = self.get_team_hierarchy_links(params.clone()).await?;
    ...
    /// List all team hierarchy links that match the provided filters.
    pub async fn get_team_hierarchy_links_with_http_info(
        &self,
        params: GetTeamHierarchyLinksOptionalParams,
    ) -> Result<
    ...
    > {
        let local_configuration = &self.config;
        let operation_id = "v2.get_team_hierarchy_links";
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            ...
            let local_entity: Option<GetTeamHierarchyLinksError> =
                serde_json::from_str(&local_content).ok();
            let local_error = datadog::ResponseContent {
                status: local_status,
                content: local_content,
