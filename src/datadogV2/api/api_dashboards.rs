...
/// ValidateDashboardWidgetsError is a struct for typed errors of method [`DashboardsAPI::validate_dashboard_widgets`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ValidateDashboardWidgetsError {
    APIErrorResponse(crate::datadogV2::model::APIErrorResponse),
    UnknownValue(serde_json::Value),
}
...
impl DashboardsAPI {
    ...
    pub async fn validate_dashboard_widgets(
        &self,
        body: crate::datadogV2::model::DashboardWidgetValidationRequest,
    ) -> Result<
        crate::datadogV2::model::DashboardWidgetValidationResponse,
        datadog::Error<ValidateDashboardWidgetsError>,
    > {
        match self.validate_dashboard_widgets_with_http_info(body).await {
            Ok(response_content) => {
                if let Some(e) = response_content.entity {
                    Ok(e)
                } else {
    ...
    /// Validate dashboard widgets without creating or updating a dashboard.
    pub async fn validate_dashboard_widgets_with_http_info(
        &self,
        body: crate::datadogV2::model::DashboardWidgetValidationRequest,
    ) -> Result<
        datadog::ResponseContent<crate::datadogV2::model::DashboardWidgetValidationResponse>,
        datadog::Error<ValidateDashboardWidgetsError>,
    > {
        let local_configuration = &self.config;
        let local_operation_id = "v2.validate_dashboard_widgets";
        if local_configuration.is_unstable_operation_enabled(local_operation_id) {
            warn!("Using unstable operation {local_operation_id}");
        } else {
            let local_error = datadog::UnstableOperationDisabledError {
                msg: "Operation 'v2.validate_dashboard_widgets' is not enabled".to_string(),
            };
            return Err(datadog::Error::UnstableOperationDisabledError(local_error));
        }
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            match serde_json::from_str::<crate::datadogV2::model::DashboardWidgetValidationResponse>(
                &local_content,
            ) {
                Ok(e) => {
                    return Ok(datadog::ResponseContent {
            ...
            let local_entity: Option<ValidateDashboardWidgetsError> =
                serde_json::from_str(&local_content).ok();
            let local_error = datadog::ResponseContent {
                status: local_status,
                content: local_content,
