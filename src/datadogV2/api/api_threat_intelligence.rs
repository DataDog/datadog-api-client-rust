...
/// IngestStixThreatIntelOptionalParams is a struct for passing parameters to the method [`ThreatIntelligenceAPI::ingest_stix_threat_intel`]
#[non_exhaustive]
#[derive(Clone, Default, Debug)]
pub struct IngestStixThreatIntelOptionalParams {
...
impl IngestStixThreatIntelOptionalParams {
    /// Content encoding for the request body. Use gzip for a compressed STIX bundle.
...
/// IngestStixThreatIntelError is a struct for typed errors of method [`ThreatIntelligenceAPI::ingest_stix_threat_intel`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum IngestStixThreatIntelError {
...
impl ThreatIntelligenceAPI {
    ...
    pub async fn ingest_stix_threat_intel(
        &self,
        ti_vendor: String,
        body: crate::datadogV2::model::STIXBundleRequest,
        params: IngestStixThreatIntelOptionalParams,
    ) -> Result<
        crate::datadogV2::model::STIXIngestResponse,
        datadog::Error<IngestStixThreatIntelError>,
    > {
        match self
            .ingest_stix_threat_intel_with_http_info(ti_vendor, body, params)
            .await
        {
            Ok(response_content) => {
    ...
    ///
    /// A successful response means ingestion has completed. Reference-table materialization and enrichment happen asynchronously. Requests are limited to 50 MB and 10 requests per second per API key. Gzip-compressed request bodies are supported.
    pub async fn ingest_stix_threat_intel_with_http_info(
        &self,
        ti_vendor: String,
        body: crate::datadogV2::model::STIXBundleRequest,
        params: IngestStixThreatIntelOptionalParams,
    ...
    > {
        ...
        let local_operation_id = "v2.ingest_stix_threat_intel";
        if local_configuration.is_unstable_operation_enabled(local_operation_id) {
            warn!("Using unstable operation {local_operation_id}");
        } else {
            let local_error = datadog::UnstableOperationDisabledError {
                msg: "Operation 'v2.ingest_stix_threat_intel' is not enabled".to_string(),
            };
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            ...
            let local_entity: Option<IngestStixThreatIntelError> =
