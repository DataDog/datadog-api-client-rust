...
/// GenerateNewExternalIDError is a struct for typed errors of method [`AWSIntegrationAPI::generate_new_external_id`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GenerateNewExternalIDError {
    APIErrorResponse(crate::datadogV2::model::APIErrorResponse),
    UnknownValue(serde_json::Value),
}
...
/// ListAWSLogServicesError is a struct for typed errors of method [`AWSIntegrationAPI::list_aws_log_services`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListAWSLogServicesError {
    APIErrorResponse(crate::datadogV2::model::APIErrorResponse),
    UnknownValue(serde_json::Value),
}
...
impl AWSIntegrationAPI {
    ...
    pub async fn generate_new_external_id(
        &self,
    ) -> Result<
        crate::datadogV2::model::AWSNewExternalIDResponse,
        datadog::Error<GenerateNewExternalIDError>,
    > {
        match self.generate_new_external_id_with_http_info().await {
            Ok(response_content) => {
                if let Some(e) = response_content.entity {
                    Ok(e)
                } else {
    ...
    /// Generate a new external ID for AWS role-based authentication.
    pub async fn generate_new_external_id_with_http_info(
        &self,
    ) -> Result<
    ...
    > {
        let local_configuration = &self.config;
        let operation_id = "v2.generate_new_external_id";
        if local_configuration.is_unstable_operation_enabled(operation_id) {
            warn!("Using unstable operation {operation_id}");
        } else {
            let local_error = datadog::UnstableOperationDisabledError {
                msg: "Operation 'v2.generate_new_external_id' is not enabled".to_string(),
            };
            return Err(datadog::Error::UnstableOperationDisabledError(local_error));
        }
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            ...
            let local_entity: Option<GenerateNewExternalIDError> =
                serde_json::from_str(&local_content).ok();
            let local_error = datadog::ResponseContent {
                status: local_status,
                content: local_content,
    ...
    pub async fn list_aws_log_services(
        &self,
    ) -> Result<
        crate::datadogV2::model::AWSLogsServicesResponse,
        datadog::Error<ListAWSLogServicesError>,
    > {
        match self.list_aws_log_services_with_http_info().await {
            Ok(response_content) => {
                if let Some(e) = response_content.entity {
                    Ok(e)
                } else {
    ...
    /// Get a list of AWS services that can send logs to Datadog.
    pub async fn list_aws_log_services_with_http_info(
        &self,
    ) -> Result<
    ...
    > {
        let local_configuration = &self.config;
        let operation_id = "v2.list_aws_log_services";
        if local_configuration.is_unstable_operation_enabled(operation_id) {
            warn!("Using unstable operation {operation_id}");
        } else {
            let local_error = datadog::UnstableOperationDisabledError {
                msg: "Operation 'v2.list_aws_log_services' is not enabled".to_string(),
            };
            return Err(datadog::Error::UnstableOperationDisabledError(local_error));
        }
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            ...
            let local_entity: Option<ListAWSLogServicesError> =
                serde_json::from_str(&local_content).ok();
            let local_error = datadog::ResponseContent {
                status: local_status,
                content: local_content,
    ...
    > {
        ...
        let local_uri_str = format!(
            "{}/api/v2/integration/aws/namespaces",
            local_configuration.get_operation_host(operation_id)
        );
        let mut local_req_builder =
            local_client.request(reqwest::Method::GET, local_uri_str.as_str());
    ...
    pub async fn update_aws_account(
        &self,
        aws_account_config_id: String,
        body: crate::datadogV2::model::AWSAccountPatchRequest,
