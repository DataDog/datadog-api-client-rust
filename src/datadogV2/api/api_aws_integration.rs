...
impl AWSIntegrationAPI {
    ...
    pub async fn delete_aws_account(
        ...
        match self.delete_aws_account_with_http_info(aws_account_id).await {
            Ok(_) => Ok(()),
            Err(err) => Err(err),
        }
    }
    ...
    pub async fn delete_aws_account_with_http_info(
        ...
        let local_uri_str = format!(
            ...
            local_configuration.get_operation_host(operation_id),
            aws_account_id = datadog::urlencode(aws_account_id)
        );
        let mut local_req_builder =
    ...
    {
        match self.get_aws_account_with_http_info(aws_account_id).await {
            Ok(response_content) => {
                if let Some(e) = response_content.entity {
                    Ok(e)
                } else {
    ...
    {
        match self.list_aws_accounts_with_http_info().await {
            Ok(response_content) => {
                if let Some(e) = response_content.entity {
                    Ok(e)
                } else {
    ...
    {
        match self
            .update_aws_account_with_http_info(aws_account_id, body)
            .await
        {
            Ok(response_content) => {
                if let Some(e) = response_content.entity {
