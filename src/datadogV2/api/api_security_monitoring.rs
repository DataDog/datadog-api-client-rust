...
/// AssignIntegrationIssuesError is a struct for typed errors of method [`SecurityMonitoringAPI::assign_integration_issues`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum AssignIntegrationIssuesError {
    JSONAPIErrorResponse(crate::datadogV2::model::JSONAPIErrorResponse),
    APIErrorResponse(crate::datadogV2::model::APIErrorResponse),
    UnknownValue(serde_json::Value),
}
...
/// CreateJiraIssueError is a struct for typed errors of method [`SecurityMonitoringAPI::create_jira_issue`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateJiraIssueError {
    JSONAPIErrorResponse(crate::datadogV2::model::JSONAPIErrorResponse),
    APIErrorResponse(crate::datadogV2::model::APIErrorResponse),
    UnknownValue(serde_json::Value),
}
...
/// GetJiraIssueMetadataError is a struct for typed errors of method [`SecurityMonitoringAPI::get_jira_issue_metadata`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetJiraIssueMetadataError {
    JSONAPIErrorResponse(crate::datadogV2::model::JSONAPIErrorResponse),
    APIErrorResponse(crate::datadogV2::model::APIErrorResponse),
    UnknownValue(serde_json::Value),
}
...
impl SecurityMonitoringAPI {
    ...
    pub async fn assign_integration_issues(
        &self,
        body: crate::datadogV2::model::IntegrationAssignmentRequest,
    ) -> Result<(), datadog::Error<AssignIntegrationIssuesError>> {
        match self.assign_integration_issues_with_http_info(body).await {
            Ok(_) => Ok(()),
            Err(err) => Err(err),
        }
    }
    ...
    /// Assign or un-assign Jira issues to security findings or vulnerabilities.
    /// This endpoint allows you to associate existing Jira issues with security findings or vulnerabilities, or remove such associations.
    pub async fn assign_integration_issues_with_http_info(
        &self,
        body: crate::datadogV2::model::IntegrationAssignmentRequest,
    ) -> Result<datadog::ResponseContent<()>, datadog::Error<AssignIntegrationIssuesError>> {
        let local_configuration = &self.config;
        let operation_id = "v2.assign_integration_issues";
        if local_configuration.is_unstable_operation_enabled(operation_id) {
            warn!("Using unstable operation {operation_id}");
        } else {
            let local_error = datadog::UnstableOperationDisabledError {
                msg: "Operation 'v2.assign_integration_issues' is not enabled".to_string(),
            };
            return Err(datadog::Error::UnstableOperationDisabledError(local_error));
        }
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            ...
            let local_entity: Option<AssignIntegrationIssuesError> =
                serde_json::from_str(&local_content).ok();
            let local_error = datadog::ResponseContent {
                status: local_status,
                content: local_content,
    ...
    pub async fn create_jira_issue(
        &self,
        body: crate::datadogV2::model::JiraIssueRequest,
    ) -> Result<(), datadog::Error<CreateJiraIssueError>> {
        match self.create_jira_issue_with_http_info(body).await {
            Ok(_) => Ok(()),
            Err(err) => Err(err),
        }
    }
    ...
    /// Create Jira issues for security findings or vulnerabilities.
    /// This endpoint creates new Jira issues based on the provided security findings or vulnerability information. The operation is asynchronous and returns immediately with a 202 Accepted status.
    pub async fn create_jira_issue_with_http_info(
        &self,
        body: crate::datadogV2::model::JiraIssueRequest,
    ) -> Result<datadog::ResponseContent<()>, datadog::Error<CreateJiraIssueError>> {
        let local_configuration = &self.config;
        let operation_id = "v2.create_jira_issue";
        if local_configuration.is_unstable_operation_enabled(operation_id) {
            warn!("Using unstable operation {operation_id}");
        } else {
            let local_error = datadog::UnstableOperationDisabledError {
                msg: "Operation 'v2.create_jira_issue' is not enabled".to_string(),
            };
            return Err(datadog::Error::UnstableOperationDisabledError(local_error));
        }
        ...
        let local_uri_str = format!(
            "{}/api/v2/cloud_security_management/jira_issues",
            local_configuration.get_operation_host(operation_id)
        );
        let mut local_req_builder =
            local_client.request(reqwest::Method::POST, local_uri_str.as_str());
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            ...
            let local_entity: Option<CreateJiraIssueError> =
                serde_json::from_str(&local_content).ok();
            let local_error = datadog::ResponseContent {
                status: local_status,
                content: local_content,
    ...
    pub async fn get_jira_issue_metadata(
        &self,
        url: String,
    ) -> Result<
        crate::datadogV2::model::JiraIssuesMetadataResponse,
        datadog::Error<GetJiraIssueMetadataError>,
    > {
        match self.get_jira_issue_metadata_with_http_info(url).await {
            Ok(response_content) => {
                if let Some(e) = response_content.entity {
                    Ok(e)
                } else {
    ...
    /// Retrieve metadata for a Jira issue.
    /// This endpoint returns metadata about a Jira issue, including account ID, issue type ID, and project ID, based on the provided Jira issue URL.
    pub async fn get_jira_issue_metadata_with_http_info(
        &self,
        url: String,
    ) -> Result<
        datadog::ResponseContent<crate::datadogV2::model::JiraIssuesMetadataResponse>,
        datadog::Error<GetJiraIssueMetadataError>,
    > {
        let local_configuration = &self.config;
        let operation_id = "v2.get_jira_issue_metadata";
        if local_configuration.is_unstable_operation_enabled(operation_id) {
            warn!("Using unstable operation {operation_id}");
        } else {
            let local_error = datadog::UnstableOperationDisabledError {
                msg: "Operation 'v2.get_jira_issue_metadata' is not enabled".to_string(),
            };
            return Err(datadog::Error::UnstableOperationDisabledError(local_error));
        }
        ...
        let local_uri_str = format!(
            "{}/api/v2/security/findings/jira_issues/metadata",
            local_configuration.get_operation_host(operation_id)
        );
        let mut local_req_builder =
            local_client.request(reqwest::Method::GET, local_uri_str.as_str());

        local_req_builder = local_req_builder.query(&[("url", &url.to_string())]);
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            match serde_json::from_str::<crate::datadogV2::model::JiraIssuesMetadataResponse>(
                &local_content,
            ) {
                Ok(e) => {
                    return Ok(datadog::ResponseContent {
            ...
            let local_entity: Option<GetJiraIssueMetadataError> =
                serde_json::from_str(&local_content).ok();
            let local_error = datadog::ResponseContent {
                status: local_status,
                content: local_content,
