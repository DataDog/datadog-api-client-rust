...
/// GetOnCallEventEmailAddressesError is a struct for typed errors of method [`EventsAPI::get_on_call_event_email_addresses`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetOnCallEventEmailAddressesError {
    JSONAPIErrorResponse(crate::datadogV2::model::JSONAPIErrorResponse),
    APIErrorResponse(crate::datadogV2::model::APIErrorResponse),
    UnknownValue(serde_json::Value),
}
...
impl EventsAPI {
    ...
    pub async fn get_on_call_event_email_addresses(
        &self,
        filter_team_handle: String,
    ) -> Result<
        crate::datadogV2::model::EventEmailAddressesResponse,
        datadog::Error<GetOnCallEventEmailAddressesError>,
    > {
        match self
            .get_on_call_event_email_addresses_with_http_info(filter_team_handle)
            .await
        {
            Ok(response_content) => {
                if let Some(e) = response_content.entity {
    ...
    /// List all on-call event email addresses for a given team handle.
    pub async fn get_on_call_event_email_addresses_with_http_info(
        &self,
        filter_team_handle: String,
    ) -> Result<
    ...
    > {
        let local_configuration = &self.config;
        let operation_id = "v2.get_on_call_event_email_addresses";
        if local_configuration.is_unstable_operation_enabled(operation_id) {
            warn!("Using unstable operation {operation_id}");
        } else {
            let local_error = datadog::UnstableOperationDisabledError {
                msg: "Operation 'v2.get_on_call_event_email_addresses' is not enabled".to_string(),
            };
            return Err(datadog::Error::UnstableOperationDisabledError(local_error));
        }
        ...
        if !local_status.is_client_error() && !local_status.is_server_error() {
            ...
            let local_entity: Option<GetOnCallEventEmailAddressesError> =
                serde_json::from_str(&local_content).ok();
            let local_error = datadog::ResponseContent {
                status: local_status,
                content: local_content,
