...
impl<'de> Deserialize<'de> for OutcomesBatchRequest {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct OutcomesBatchRequestVisitor;
        impl<'a> Visitor<'a> for OutcomesBatchRequestVisitor {
            type Value = OutcomesBatchRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::OutcomesBatchRequestData> = None;
                ...
                let content = OutcomesBatchRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(OutcomesBatchRequestVisitor)
    }
}
