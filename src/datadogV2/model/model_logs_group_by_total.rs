...
impl<'de> Deserialize<'de> for LogsGroupByTotal {
        ...
        D: Deserializer<'de>,
    {
        ...
        if let Ok(_v) = serde_json::from_value::<bool>(value.clone()) {
            return Ok(LogsGroupByTotal::LogsGroupByTotalBoolean(_v));
        }
        if let Ok(_v) = serde_json::from_value::<String>(value.clone()) {
            return Ok(LogsGroupByTotal::LogsGroupByTotalString(_v));
        }
        if let Ok(_v) = serde_json::from_value::<f64>(value.clone()) {
            return Ok(LogsGroupByTotal::LogsGroupByTotalNumber(_v));
        }
