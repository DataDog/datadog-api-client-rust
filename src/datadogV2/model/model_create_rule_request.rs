...
impl<'de> Deserialize<'de> for CreateRuleRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct CreateRuleRequestVisitor;
        impl<'a> Visitor<'a> for CreateRuleRequestVisitor {
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::CreateRuleRequestData> = None;
                ...
                let content = CreateRuleRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(CreateRuleRequestVisitor)
    }
}
