...
impl<'de> Deserialize<'de> for MetricBulkTagConfigResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct MetricBulkTagConfigResponseVisitor;
        impl<'a> Visitor<'a> for MetricBulkTagConfigResponseVisitor {
            type Value = MetricBulkTagConfigResponse;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::MetricBulkTagConfigStatus> = None;
                ...
                let content = MetricBulkTagConfigResponse { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(MetricBulkTagConfigResponseVisitor)
    }
}
