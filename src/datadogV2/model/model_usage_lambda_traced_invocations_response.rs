...
impl<'de> Deserialize<'de> for UsageLambdaTracedInvocationsResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct UsageLambdaTracedInvocationsResponseVisitor;
        impl<'a> Visitor<'a> for UsageLambdaTracedInvocationsResponseVisitor {
            type Value = UsageLambdaTracedInvocationsResponse;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = UsageLambdaTracedInvocationsResponse { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(UsageLambdaTracedInvocationsResponseVisitor)
    }
}
