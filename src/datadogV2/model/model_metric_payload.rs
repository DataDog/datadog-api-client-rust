...
impl<'de> Deserialize<'de> for MetricPayload {
    ...
    where
        D: Deserializer<'de>,
    {
        struct MetricPayloadVisitor;
        impl<'a> Visitor<'a> for MetricPayloadVisitor {
            type Value = MetricPayload;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut series: Option<Vec<crate::datadogV2::model::MetricSeries>> = None;
                ...
                let content = MetricPayload { series, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(MetricPayloadVisitor)
    }
}
