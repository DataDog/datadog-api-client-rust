...
impl<'de> Deserialize<'de> for OktaAccountUpdateRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct OktaAccountUpdateRequestVisitor;
        impl<'a> Visitor<'a> for OktaAccountUpdateRequestVisitor {
            type Value = OktaAccountUpdateRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::OktaAccountUpdateRequestData> = None;
                ...
                let content = OktaAccountUpdateRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(OktaAccountUpdateRequestVisitor)
    }
}
