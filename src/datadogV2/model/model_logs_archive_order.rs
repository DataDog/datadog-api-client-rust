...
impl<'de> Deserialize<'de> for LogsArchiveOrder {
    ...
    where
        D: Deserializer<'de>,
    {
        struct LogsArchiveOrderVisitor;
        impl<'a> Visitor<'a> for LogsArchiveOrderVisitor {
            type Value = LogsArchiveOrder;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::LogsArchiveOrderDefinition> = None;
                ...
                let content = LogsArchiveOrder { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(LogsArchiveOrderVisitor)
    }
}
