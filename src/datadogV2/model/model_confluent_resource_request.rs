...
impl<'de> Deserialize<'de> for ConfluentResourceRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct ConfluentResourceRequestVisitor;
        impl<'a> Visitor<'a> for ConfluentResourceRequestVisitor {
            type Value = ConfluentResourceRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::ConfluentResourceRequestData> = None;
                ...
                let content = ConfluentResourceRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(ConfluentResourceRequestVisitor)
    }
}
