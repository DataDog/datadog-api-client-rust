// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.
use serde::de::{Error, MapAccess, Visitor};
use serde::{Deserialize, Deserializer, Serialize};
use serde_with::skip_serializing_none;
use std::fmt::{self, Formatter};

/// A journey-based query block within a segment data query.
#[non_exhaustive]
#[skip_serializing_none]
#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct RumSegmentJourney {
    /// The type of conversion to track.
    #[serde(rename = "conversion_type")]
    pub conversion_type: Option<String>,
    /// The facet to group journey results by.
    #[serde(rename = "group_by")]
    pub group_by: Option<String>,
    /// The name of this journey query block.
    #[serde(rename = "name")]
    pub name: Option<String>,
    /// The search query for filtering events.
    #[serde(rename = "search")]
    pub search: Option<String>,
    #[serde(flatten)]
    pub additional_properties: std::collections::BTreeMap<String, serde_json::Value>,
    #[serde(skip)]
    #[serde(default)]
    pub(crate) _unparsed: bool,
}

impl RumSegmentJourney {
    pub fn new() -> RumSegmentJourney {
        RumSegmentJourney {
            conversion_type: None,
            group_by: None,
            name: None,
            search: None,
            additional_properties: std::collections::BTreeMap::new(),
            _unparsed: false,
        }
    }

    pub fn conversion_type(mut self, value: String) -> Self {
        self.conversion_type = Some(value);
        self
    }

    pub fn group_by(mut self, value: String) -> Self {
        self.group_by = Some(value);
        self
    }

    pub fn name(mut self, value: String) -> Self {
        self.name = Some(value);
        self
    }

    pub fn search(mut self, value: String) -> Self {
        self.search = Some(value);
        self
    }

    pub fn additional_properties(
        mut self,
        value: std::collections::BTreeMap<String, serde_json::Value>,
    ) -> Self {
        self.additional_properties = value;
        self
    }
}

impl Default for RumSegmentJourney {
    fn default() -> Self {
        Self::new()
    }
}

impl<'de> Deserialize<'de> for RumSegmentJourney {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct RumSegmentJourneyVisitor;
        impl<'a> Visitor<'a> for RumSegmentJourneyVisitor {
            type Value = RumSegmentJourney;

            fn expecting(&self, f: &mut Formatter<'_>) -> fmt::Result {
                f.write_str("a mapping")
            }

            fn visit_map<M>(self, mut map: M) -> Result<Self::Value, M::Error>
            where
                M: MapAccess<'a>,
            {
                let mut conversion_type: Option<String> = None;
                let mut group_by: Option<String> = None;
                let mut name: Option<String> = None;
                let mut search: Option<String> = None;
                let mut additional_properties: std::collections::BTreeMap<
                    String,
                    serde_json::Value,
                > = std::collections::BTreeMap::new();
                let mut _unparsed = false;

                while let Some((k, v)) = map.next_entry::<String, serde_json::Value>()? {
                    match k.as_str() {
                        "conversion_type" => {
                            if v.is_null() {
                                continue;
                            }
                            conversion_type =
                                Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "group_by" => {
                            if v.is_null() {
                                continue;
                            }
                            group_by = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "name" => {
                            if v.is_null() {
                                continue;
                            }
                            name = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "search" => {
                            if v.is_null() {
                                continue;
                            }
                            search = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        &_ => {
                            if let Ok(value) = serde_json::from_value(v.clone()) {
                                additional_properties.insert(k, value);
                            }
                        }
                    }
                }

                let content = RumSegmentJourney {
                    conversion_type,
                    group_by,
                    name,
                    search,
                    additional_properties,
                    _unparsed,
                };

                Ok(content)
            }
        }

        deserializer.deserialize_any(RumSegmentJourneyVisitor)
    }
}
