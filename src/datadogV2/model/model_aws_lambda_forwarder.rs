// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.
use serde::de::{Error, MapAccess, Visitor};
use serde::{Deserialize, Deserializer, Serialize};
use serde_with::skip_serializing_none;
use std::fmt::{self, Formatter};

/// AWS Lambda forwarder
#[non_exhaustive]
#[skip_serializing_none]
#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct AWSLambdaForwarder {
    /// List of Datadog Lambda Log Forwarder ARNs
    #[serde(rename = "lambdas")]
    pub lambdas: Option<Vec<String>>,
    /// List of AWS services that will send logs to the Datadog Lambda Log Forwarder
    #[serde(rename = "sources")]
    pub sources: Option<Vec<String>>,
    #[serde(skip)]
    #[serde(default)]
    pub(crate) _unparsed: bool,
}

impl AWSLambdaForwarder {
    pub fn new() -> AWSLambdaForwarder {
        AWSLambdaForwarder {
            lambdas: None,
            sources: None,
            _unparsed: false,
        }
    }

    pub fn lambdas(mut self, value: Vec<String>) -> Self {
        self.lambdas = Some(value);
        self
    }

    pub fn sources(mut self, value: Vec<String>) -> Self {
        self.sources = Some(value);
        self
    }
}

impl Default for AWSLambdaForwarder {
    fn default() -> Self {
        Self::new()
    }
}

impl<'de> Deserialize<'de> for AWSLambdaForwarder {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct AWSLambdaForwarderVisitor;
        impl<'a> Visitor<'a> for AWSLambdaForwarderVisitor {
            type Value = AWSLambdaForwarder;

            fn expecting(&self, f: &mut Formatter<'_>) -> fmt::Result {
                f.write_str("a mapping")
            }

            fn visit_map<M>(self, mut map: M) -> Result<Self::Value, M::Error>
            where
                M: MapAccess<'a>,
            {
                let mut lambdas: Option<Vec<String>> = None;
                let mut sources: Option<Vec<String>> = None;
                let mut _unparsed = false;

                while let Some((k, v)) = map.next_entry::<String, serde_json::Value>()? {
                    match k.as_str() {
                        "lambdas" => {
                            if v.is_null() {
                                continue;
                            }
                            lambdas = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "sources" => {
                            if v.is_null() {
                                continue;
                            }
                            sources = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        &_ => {}
                    }
                }

                let content = AWSLambdaForwarder {
                    lambdas,
                    sources,
                    _unparsed,
                };

                Ok(content)
            }
        }

        deserializer.deserialize_any(AWSLambdaForwarderVisitor)
    }
}
