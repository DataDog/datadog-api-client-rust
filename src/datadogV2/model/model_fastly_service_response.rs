...
impl<'de> Deserialize<'de> for FastlyServiceResponse {
    ...
    where
        D: Deserializer<'de>,
    {
        struct FastlyServiceResponseVisitor;
        impl<'a> Visitor<'a> for FastlyServiceResponseVisitor {
            type Value = FastlyServiceResponse;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = FastlyServiceResponse { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(FastlyServiceResponseVisitor)
    }
}
