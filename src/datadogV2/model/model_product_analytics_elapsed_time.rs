// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.
use serde::de::{Error, MapAccess, Visitor};
use serde::{Deserialize, Deserializer, Serialize};
use serde_with::skip_serializing_none;
use std::fmt::{self, Formatter};

/// Elapsed time statistics (min/max/avg in milliseconds).
#[non_exhaustive]
#[skip_serializing_none]
#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct ProductAnalyticsElapsedTime {
    /// Average elapsed time to reach the next step, in milliseconds.
    #[serde(rename = "avg")]
    pub avg: i64,
    /// Maximum elapsed time to reach the next step, in milliseconds.
    #[serde(rename = "max")]
    pub max: i64,
    /// Minimum elapsed time to reach the next step, in milliseconds.
    #[serde(rename = "min")]
    pub min: i64,
    #[serde(flatten)]
    pub additional_properties: std::collections::BTreeMap<String, serde_json::Value>,
    #[serde(skip)]
    #[serde(default)]
    pub(crate) _unparsed: bool,
}

impl ProductAnalyticsElapsedTime {
    pub fn new(avg: i64, max: i64, min: i64) -> ProductAnalyticsElapsedTime {
        ProductAnalyticsElapsedTime {
            avg,
            max,
            min,
            additional_properties: std::collections::BTreeMap::new(),
            _unparsed: false,
        }
    }

    pub fn additional_properties(
        mut self,
        value: std::collections::BTreeMap<String, serde_json::Value>,
    ) -> Self {
        self.additional_properties = value;
        self
    }
}

impl<'de> Deserialize<'de> for ProductAnalyticsElapsedTime {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct ProductAnalyticsElapsedTimeVisitor;
        impl<'a> Visitor<'a> for ProductAnalyticsElapsedTimeVisitor {
            type Value = ProductAnalyticsElapsedTime;

            fn expecting(&self, f: &mut Formatter<'_>) -> fmt::Result {
                f.write_str("a mapping")
            }

            fn visit_map<M>(self, mut map: M) -> Result<Self::Value, M::Error>
            where
                M: MapAccess<'a>,
            {
                let mut avg: Option<i64> = None;
                let mut max: Option<i64> = None;
                let mut min: Option<i64> = None;
                let mut additional_properties: std::collections::BTreeMap<
                    String,
                    serde_json::Value,
                > = std::collections::BTreeMap::new();
                let mut _unparsed = false;

                while let Some((k, v)) = map.next_entry::<String, serde_json::Value>()? {
                    match k.as_str() {
                        "avg" => {
                            avg = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "max" => {
                            max = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "min" => {
                            min = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        &_ => {
                            if let Ok(value) = serde_json::from_value(v.clone()) {
                                additional_properties.insert(k, value);
                            }
                        }
                    }
                }
                let avg = avg.ok_or_else(|| M::Error::missing_field("avg"))?;
                let max = max.ok_or_else(|| M::Error::missing_field("max"))?;
                let min = min.ok_or_else(|| M::Error::missing_field("min"))?;

                let content = ProductAnalyticsElapsedTime {
                    avg,
                    max,
                    min,
                    additional_properties,
                    _unparsed,
                };

                Ok(content)
            }
        }

        deserializer.deserialize_any(ProductAnalyticsElapsedTimeVisitor)
    }
}
