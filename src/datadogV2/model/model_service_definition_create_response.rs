...
impl<'de> Deserialize<'de> for ServiceDefinitionCreateResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct ServiceDefinitionCreateResponseVisitor;
        impl<'a> Visitor<'a> for ServiceDefinitionCreateResponseVisitor {
            type Value = ServiceDefinitionCreateResponse;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = ServiceDefinitionCreateResponse { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(ServiceDefinitionCreateResponseVisitor)
    }
}
