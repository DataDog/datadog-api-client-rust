// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.
use serde::de::{Error, MapAccess, Visitor};
use serde::{Deserialize, Deserializer, Serialize};
use serde_with::skip_serializing_none;
use std::fmt::{self, Formatter};

/// Metadata accompanying the spans response.
#[non_exhaustive]
#[skip_serializing_none]
#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct LLMObsSpansResponseMeta {
    /// Time elapsed for the query in milliseconds.
    #[serde(rename = "elapsed")]
    pub elapsed: i64,
    /// Pagination cursor for the spans response.
    #[serde(rename = "page")]
    pub page: crate::datadogV2::model::LLMObsSpansResponsePage,
    /// Unique identifier for the request.
    #[serde(rename = "request_id")]
    pub request_id: String,
    /// Status of the query execution.
    #[serde(rename = "status")]
    pub status: String,
    #[serde(flatten)]
    pub additional_properties: std::collections::BTreeMap<String, serde_json::Value>,
    #[serde(skip)]
    #[serde(default)]
    pub(crate) _unparsed: bool,
}

impl LLMObsSpansResponseMeta {
    pub fn new(
        elapsed: i64,
        page: crate::datadogV2::model::LLMObsSpansResponsePage,
        request_id: String,
        status: String,
    ) -> LLMObsSpansResponseMeta {
        LLMObsSpansResponseMeta {
            elapsed,
            page,
            request_id,
            status,
            additional_properties: std::collections::BTreeMap::new(),
            _unparsed: false,
        }
    }

    pub fn additional_properties(
        mut self,
        value: std::collections::BTreeMap<String, serde_json::Value>,
    ) -> Self {
        self.additional_properties = value;
        self
    }
}

impl<'de> Deserialize<'de> for LLMObsSpansResponseMeta {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct LLMObsSpansResponseMetaVisitor;
        impl<'a> Visitor<'a> for LLMObsSpansResponseMetaVisitor {
            type Value = LLMObsSpansResponseMeta;

            fn expecting(&self, f: &mut Formatter<'_>) -> fmt::Result {
                f.write_str("a mapping")
            }

            fn visit_map<M>(self, mut map: M) -> Result<Self::Value, M::Error>
            where
                M: MapAccess<'a>,
            {
                let mut elapsed: Option<i64> = None;
                let mut page: Option<crate::datadogV2::model::LLMObsSpansResponsePage> = None;
                let mut request_id: Option<String> = None;
                let mut status: Option<String> = None;
                let mut additional_properties: std::collections::BTreeMap<
                    String,
                    serde_json::Value,
                > = std::collections::BTreeMap::new();
                let mut _unparsed = false;

                while let Some((k, v)) = map.next_entry::<String, serde_json::Value>()? {
                    match k.as_str() {
                        "elapsed" => {
                            elapsed = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "page" => {
                            page = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "request_id" => {
                            request_id = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "status" => {
                            status = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        &_ => {
                            if let Ok(value) = serde_json::from_value(v.clone()) {
                                additional_properties.insert(k, value);
                            }
                        }
                    }
                }
                let elapsed = elapsed.ok_or_else(|| M::Error::missing_field("elapsed"))?;
                let page = page.ok_or_else(|| M::Error::missing_field("page"))?;
                let request_id = request_id.ok_or_else(|| M::Error::missing_field("request_id"))?;
                let status = status.ok_or_else(|| M::Error::missing_field("status"))?;

                let content = LLMObsSpansResponseMeta {
                    elapsed,
                    page,
                    request_id,
                    status,
                    additional_properties,
                    _unparsed,
                };

                Ok(content)
            }
        }

        deserializer.deserialize_any(LLMObsSpansResponseMetaVisitor)
    }
}
