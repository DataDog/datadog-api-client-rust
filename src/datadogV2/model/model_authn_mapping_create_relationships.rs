...
impl<'de> Deserialize<'de> for AuthNMappingCreateRelationships {
    ...
    where
        D: Deserializer<'de>,
    {
        struct AuthNMappingCreateRelationshipsVisitor;
        impl<'a> Visitor<'a> for AuthNMappingCreateRelationshipsVisitor {
            type Value = AuthNMappingCreateRelationships;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = AuthNMappingCreateRelationships { role, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(AuthNMappingCreateRelationshipsVisitor)
    }
}
