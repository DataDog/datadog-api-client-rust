...
impl<'de> Deserialize<'de> for RUMResponsePage {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct RUMResponsePageVisitor;
        impl<'a> Visitor<'a> for RUMResponsePageVisitor {
            type Value = RUMResponsePage;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = RUMResponsePage { after, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(RUMResponsePageVisitor)
    }
}
