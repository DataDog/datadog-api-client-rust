...
impl<'de> Deserialize<'de> for FastlyServiceRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct FastlyServiceRequestVisitor;
        impl<'a> Visitor<'a> for FastlyServiceRequestVisitor {
            type Value = FastlyServiceRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::FastlyServiceData> = None;
                ...
                let content = FastlyServiceRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(FastlyServiceRequestVisitor)
    }
}
