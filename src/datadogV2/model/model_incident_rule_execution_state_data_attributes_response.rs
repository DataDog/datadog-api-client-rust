// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.
use serde::de::{Error, MapAccess, Visitor};
use serde::{Deserialize, Deserializer, Serialize};
use serde_with::skip_serializing_none;
use std::fmt::{self, Formatter};

/// Attributes of an incident rule execution state.
#[non_exhaustive]
#[skip_serializing_none]
#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct IncidentRuleExecutionStateDataAttributesResponse {
    /// Timestamp when the state was created.
    #[serde(rename = "created")]
    pub created: chrono::DateTime<chrono::Utc>,
    /// The incident identifier.
    #[serde(rename = "incident_uuid")]
    pub incident_uuid: uuid::Uuid,
    /// Timestamp of the last rule execution.
    #[serde(
        rename = "last_executed_at",
        default,
        with = "::serde_with::rust::double_option"
    )]
    pub last_executed_at: Option<Option<chrono::DateTime<chrono::Utc>>>,
    /// Timestamp when the state was last modified.
    #[serde(rename = "modified")]
    pub modified: chrono::DateTime<chrono::Utc>,
    /// The rule identifier.
    #[serde(rename = "rule_uuid")]
    pub rule_uuid: uuid::Uuid,
    #[serde(flatten)]
    pub additional_properties: std::collections::BTreeMap<String, serde_json::Value>,
    #[serde(skip)]
    #[serde(default)]
    pub(crate) _unparsed: bool,
}

impl IncidentRuleExecutionStateDataAttributesResponse {
    pub fn new(
        created: chrono::DateTime<chrono::Utc>,
        incident_uuid: uuid::Uuid,
        modified: chrono::DateTime<chrono::Utc>,
        rule_uuid: uuid::Uuid,
    ) -> IncidentRuleExecutionStateDataAttributesResponse {
        IncidentRuleExecutionStateDataAttributesResponse {
            created,
            incident_uuid,
            last_executed_at: None,
            modified,
            rule_uuid,
            additional_properties: std::collections::BTreeMap::new(),
            _unparsed: false,
        }
    }

    pub fn last_executed_at(mut self, value: Option<chrono::DateTime<chrono::Utc>>) -> Self {
        self.last_executed_at = Some(value);
        self
    }

    pub fn additional_properties(
        mut self,
        value: std::collections::BTreeMap<String, serde_json::Value>,
    ) -> Self {
        self.additional_properties = value;
        self
    }
}

impl<'de> Deserialize<'de> for IncidentRuleExecutionStateDataAttributesResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct IncidentRuleExecutionStateDataAttributesResponseVisitor;
        impl<'a> Visitor<'a> for IncidentRuleExecutionStateDataAttributesResponseVisitor {
            type Value = IncidentRuleExecutionStateDataAttributesResponse;

            fn expecting(&self, f: &mut Formatter<'_>) -> fmt::Result {
                f.write_str("a mapping")
            }

            fn visit_map<M>(self, mut map: M) -> Result<Self::Value, M::Error>
            where
                M: MapAccess<'a>,
            {
                let mut created: Option<chrono::DateTime<chrono::Utc>> = None;
                let mut incident_uuid: Option<uuid::Uuid> = None;
                let mut last_executed_at: Option<Option<chrono::DateTime<chrono::Utc>>> = None;
                let mut modified: Option<chrono::DateTime<chrono::Utc>> = None;
                let mut rule_uuid: Option<uuid::Uuid> = None;
                let mut additional_properties: std::collections::BTreeMap<
                    String,
                    serde_json::Value,
                > = std::collections::BTreeMap::new();
                let mut _unparsed = false;

                while let Some((k, v)) = map.next_entry::<String, serde_json::Value>()? {
                    match k.as_str() {
                        "created" => {
                            created = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "incident_uuid" => {
                            incident_uuid =
                                Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "last_executed_at" => {
                            last_executed_at =
                                Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "modified" => {
                            modified = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "rule_uuid" => {
                            rule_uuid = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        &_ => {
                            if let Ok(value) = serde_json::from_value(v.clone()) {
                                additional_properties.insert(k, value);
                            }
                        }
                    }
                }
                let created = created.ok_or_else(|| M::Error::missing_field("created"))?;
                let incident_uuid =
                    incident_uuid.ok_or_else(|| M::Error::missing_field("incident_uuid"))?;
                let modified = modified.ok_or_else(|| M::Error::missing_field("modified"))?;
                let rule_uuid = rule_uuid.ok_or_else(|| M::Error::missing_field("rule_uuid"))?;

                let content = IncidentRuleExecutionStateDataAttributesResponse {
                    created,
                    incident_uuid,
                    last_executed_at,
                    modified,
                    rule_uuid,
                    additional_properties,
                    _unparsed,
                };

                Ok(content)
            }
        }

        deserializer.deserialize_any(IncidentRuleExecutionStateDataAttributesResponseVisitor)
    }
}
