...
impl<'de> Deserialize<'de> for AnyValueItem {
        ...
        D: Deserializer<'de>,
    {
        ...
        if let Ok(_v) = serde_json::from_value::<String>(value.clone()) {
            return Ok(AnyValueItem::AnyValueString(_v));
        }
        if let Ok(_v) = serde_json::from_value::<f64>(value.clone()) {
            return Ok(AnyValueItem::AnyValueNumber(_v));
        }
        if let Ok(_v) = serde_json::from_value::<
            std::collections::BTreeMap<String, serde_json::Value>,
        >(value.clone())
        {
            return Ok(AnyValueItem::AnyValueObject(_v));
        }
        if let Ok(_v) = serde_json::from_value::<bool>(value.clone()) {
            return Ok(AnyValueItem::AnyValueBoolean(_v));
        }

        return Ok(AnyValueItem::UnparsedObject(
