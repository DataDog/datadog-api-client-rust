...
impl<'de> Deserialize<'de> for MetricEstimateResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct MetricEstimateResponseVisitor;
        impl<'a> Visitor<'a> for MetricEstimateResponseVisitor {
            type Value = MetricEstimateResponse;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::MetricEstimate> = None;
                ...
                let content = MetricEstimateResponse { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(MetricEstimateResponseVisitor)
    }
}
