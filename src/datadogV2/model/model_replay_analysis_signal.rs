// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.
use serde::de::{Error, MapAccess, Visitor};
use serde::{Deserialize, Deserializer, Serialize};
use serde_with::skip_serializing_none;
use std::fmt::{self, Formatter};

/// A signal associated with a replay issue, capturing user interaction details.
#[non_exhaustive]
#[skip_serializing_none]
#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct ReplayAnalysisSignal {
    /// Event name associated with the signal.
    #[serde(rename = "event")]
    pub event: String,
    /// Type of signal captured.
    #[serde(rename = "signal_type")]
    pub signal_type: String,
    /// Absolute timestamp of the signal in milliseconds.
    #[serde(rename = "timestamp_ms")]
    pub timestamp_ms: i64,
    /// User behavior pattern identified for the signal.
    #[serde(rename = "user_pattern")]
    pub user_pattern: String,
    #[serde(flatten)]
    pub additional_properties: std::collections::BTreeMap<String, serde_json::Value>,
    #[serde(skip)]
    #[serde(default)]
    pub(crate) _unparsed: bool,
}

impl ReplayAnalysisSignal {
    pub fn new(
        event: String,
        signal_type: String,
        timestamp_ms: i64,
        user_pattern: String,
    ) -> ReplayAnalysisSignal {
        ReplayAnalysisSignal {
            event,
            signal_type,
            timestamp_ms,
            user_pattern,
            additional_properties: std::collections::BTreeMap::new(),
            _unparsed: false,
        }
    }

    pub fn additional_properties(
        mut self,
        value: std::collections::BTreeMap<String, serde_json::Value>,
    ) -> Self {
        self.additional_properties = value;
        self
    }
}

impl<'de> Deserialize<'de> for ReplayAnalysisSignal {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct ReplayAnalysisSignalVisitor;
        impl<'a> Visitor<'a> for ReplayAnalysisSignalVisitor {
            type Value = ReplayAnalysisSignal;

            fn expecting(&self, f: &mut Formatter<'_>) -> fmt::Result {
                f.write_str("a mapping")
            }

            fn visit_map<M>(self, mut map: M) -> Result<Self::Value, M::Error>
            where
                M: MapAccess<'a>,
            {
                let mut event: Option<String> = None;
                let mut signal_type: Option<String> = None;
                let mut timestamp_ms: Option<i64> = None;
                let mut user_pattern: Option<String> = None;
                let mut additional_properties: std::collections::BTreeMap<
                    String,
                    serde_json::Value,
                > = std::collections::BTreeMap::new();
                let mut _unparsed = false;

                while let Some((k, v)) = map.next_entry::<String, serde_json::Value>()? {
                    match k.as_str() {
                        "event" => {
                            event = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "signal_type" => {
                            signal_type =
                                Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "timestamp_ms" => {
                            timestamp_ms =
                                Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        "user_pattern" => {
                            user_pattern =
                                Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        &_ => {
                            if let Ok(value) = serde_json::from_value(v.clone()) {
                                additional_properties.insert(k, value);
                            }
                        }
                    }
                }
                let event = event.ok_or_else(|| M::Error::missing_field("event"))?;
                let signal_type =
                    signal_type.ok_or_else(|| M::Error::missing_field("signal_type"))?;
                let timestamp_ms =
                    timestamp_ms.ok_or_else(|| M::Error::missing_field("timestamp_ms"))?;
                let user_pattern =
                    user_pattern.ok_or_else(|| M::Error::missing_field("user_pattern"))?;

                let content = ReplayAnalysisSignal {
                    event,
                    signal_type,
                    timestamp_ms,
                    user_pattern,
                    additional_properties,
                    _unparsed,
                };

                Ok(content)
            }
        }

        deserializer.deserialize_any(ReplayAnalysisSignalVisitor)
    }
}
