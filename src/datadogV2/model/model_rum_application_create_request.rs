...
impl<'de> Deserialize<'de> for RUMApplicationCreateRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct RUMApplicationCreateRequestVisitor;
        impl<'a> Visitor<'a> for RUMApplicationCreateRequestVisitor {
            type Value = RUMApplicationCreateRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::RUMApplicationCreate> = None;
                ...
                let content = RUMApplicationCreateRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(RUMApplicationCreateRequestVisitor)
    }
}
