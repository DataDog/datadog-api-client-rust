...
impl<'de> Deserialize<'de> for ConfluentAccountCreateRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct ConfluentAccountCreateRequestVisitor;
        impl<'a> Visitor<'a> for ConfluentAccountCreateRequestVisitor {
            type Value = ConfluentAccountCreateRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::ConfluentAccountCreateRequestData> =
                    None;
                ...
                let content = ConfluentAccountCreateRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(ConfluentAccountCreateRequestVisitor)
    }
}
