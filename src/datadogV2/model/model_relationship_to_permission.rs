...
impl<'de> Deserialize<'de> for RelationshipToPermission {
    ...
    where
        D: Deserializer<'de>,
    {
        struct RelationshipToPermissionVisitor;
        impl<'a> Visitor<'a> for RelationshipToPermissionVisitor {
            type Value = RelationshipToPermission;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::RelationshipToPermissionData> = None;
                ...
                let content = RelationshipToPermission { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(RelationshipToPermissionVisitor)
    }
}
