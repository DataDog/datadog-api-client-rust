...
impl<'de> Deserialize<'de> for RUMApplicationUpdateRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct RUMApplicationUpdateRequestVisitor;
        impl<'a> Visitor<'a> for RUMApplicationUpdateRequestVisitor {
            type Value = RUMApplicationUpdateRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::RUMApplicationUpdate> = None;
                ...
                let content = RUMApplicationUpdateRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(RUMApplicationUpdateRequestVisitor)
    }
}
