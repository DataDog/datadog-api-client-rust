// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.
use serde::de::{Error, MapAccess, Visitor};
use serde::{Deserialize, Deserializer, Serialize};
use serde_with::skip_serializing_none;
use std::fmt::{self, Formatter};

/// The JSON:API relationship to a scorecard rule.
#[non_exhaustive]
#[skip_serializing_none]
#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct RuleOutcomeRelationships {
    /// The JSON:API relationship to a scorecard outcome.
    #[serde(rename = "rule")]
    pub rule: Option<crate::datadogV2::model::RelationshipToOutcome>,
    #[serde(skip)]
    #[serde(default)]
    pub(crate) _unparsed: bool,
}

impl RuleOutcomeRelationships {
    pub fn new() -> RuleOutcomeRelationships {
        RuleOutcomeRelationships {
            rule: None,
            _unparsed: false,
        }
    }

    pub fn rule(mut self, value: crate::datadogV2::model::RelationshipToOutcome) -> Self {
        self.rule = Some(value);
        self
    }
}

impl Default for RuleOutcomeRelationships {
    fn default() -> Self {
        Self::new()
    }
}

impl<'de> Deserialize<'de> for RuleOutcomeRelationships {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct RuleOutcomeRelationshipsVisitor;
        impl<'a> Visitor<'a> for RuleOutcomeRelationshipsVisitor {
            type Value = RuleOutcomeRelationships;

            fn expecting(&self, f: &mut Formatter<'_>) -> fmt::Result {
                f.write_str("a mapping")
            }

            fn visit_map<M>(self, mut map: M) -> Result<Self::Value, M::Error>
            where
                M: MapAccess<'a>,
            {
                let mut rule: Option<crate::datadogV2::model::RelationshipToOutcome> = None;
                let mut _unparsed = false;

                while let Some((k, v)) = map.next_entry::<String, serde_json::Value>()? {
                    match k.as_str() {
                        "rule" => {
                            if v.is_null() {
                                continue;
                            }
                            rule = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        &_ => {}
                    }
                }

                let content = RuleOutcomeRelationships { rule, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(RuleOutcomeRelationshipsVisitor)
    }
}
