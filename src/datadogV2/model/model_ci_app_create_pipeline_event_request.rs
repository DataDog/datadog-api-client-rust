...
impl<'de> Deserialize<'de> for CIAppCreatePipelineEventRequest {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct CIAppCreatePipelineEventRequestVisitor;
        impl<'a> Visitor<'a> for CIAppCreatePipelineEventRequestVisitor {
            type Value = CIAppCreatePipelineEventRequest;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = CIAppCreatePipelineEventRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(CIAppCreatePipelineEventRequestVisitor)
    }
}
