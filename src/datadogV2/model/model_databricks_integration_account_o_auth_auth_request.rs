...
/// Databricks OAuth machine-to-machine authentication using a service principal. Because an update replaces this object entirely, every required field must be sent again on each update, and omitting `azure_tenant_id` clears it.
#[non_exhaustive]
