...
impl<'de> Deserialize<'de> for FastlyAccountUpdateRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct FastlyAccountUpdateRequestVisitor;
        impl<'a> Visitor<'a> for FastlyAccountUpdateRequestVisitor {
            type Value = FastlyAccountUpdateRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::FastlyAccountUpdateRequestData> =
                    None;
                ...
                let content = FastlyAccountUpdateRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(FastlyAccountUpdateRequestVisitor)
    }
}
