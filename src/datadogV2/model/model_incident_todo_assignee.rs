// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.
use serde::{Deserialize, Deserializer, Serialize};

/// A todo assignee.
#[non_exhaustive]
#[derive(Clone, Debug, PartialEq, Serialize)]
#[serde(untagged)]
pub enum IncidentTodoAssignee {
    IncidentTodoAssigneeHandle(String),
    IncidentTodoAnonymousAssignee(Box<crate::datadogV2::model::IncidentTodoAnonymousAssignee>),
    UnparsedObject(crate::datadog::UnparsedObejct),
}

impl<'de> Deserialize<'de> for IncidentTodoAssignee {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        let value: serde_json::Value = Deserialize::deserialize(deserializer)?;
        if let Ok(_v) = serde_json::from_value::<String>(value.clone()) {
            return Ok(IncidentTodoAssignee::IncidentTodoAssigneeHandle(_v));
        }
        if let Ok(_v) = serde_json::from_value::<
            Box<crate::datadogV2::model::IncidentTodoAnonymousAssignee>,
        >(value.clone())
        {
            if !_v._unparsed {
                return Ok(IncidentTodoAssignee::IncidentTodoAnonymousAssignee(_v));
            }
        }

        return Ok(IncidentTodoAssignee::UnparsedObject(
            crate::datadog::UnparsedObejct { value },
        ));
    }
}
