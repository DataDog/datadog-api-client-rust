...
impl<'de> Deserialize<'de> for AuthNMappingUpdateRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct AuthNMappingUpdateRequestVisitor;
        impl<'a> Visitor<'a> for AuthNMappingUpdateRequestVisitor {
            type Value = AuthNMappingUpdateRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::AuthNMappingUpdateData> = None;
                ...
                let content = AuthNMappingUpdateRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(AuthNMappingUpdateRequestVisitor)
    }
}
