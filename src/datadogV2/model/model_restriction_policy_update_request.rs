...
impl<'de> Deserialize<'de> for RestrictionPolicyUpdateRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct RestrictionPolicyUpdateRequestVisitor;
        impl<'a> Visitor<'a> for RestrictionPolicyUpdateRequestVisitor {
            type Value = RestrictionPolicyUpdateRequest;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = RestrictionPolicyUpdateRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(RestrictionPolicyUpdateRequestVisitor)
    }
}
