...
impl<'de> Deserialize<'de> for FastlyServicesResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct FastlyServicesResponseVisitor;
        impl<'a> Visitor<'a> for FastlyServicesResponseVisitor {
            type Value = FastlyServicesResponse;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = FastlyServicesResponse { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(FastlyServicesResponseVisitor)
    }
}
