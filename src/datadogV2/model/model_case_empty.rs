...
impl CaseEmpty {
    pub fn new(type_: crate::datadogV2::model::CaseResourceType) -> CaseEmpty {
        CaseEmpty {
            type_,
            _unparsed: false,
        }
...
impl<'de> Deserialize<'de> for CaseEmpty {
    ...
    where
        D: Deserializer<'de>,
    {
        struct CaseEmptyVisitor;
        impl<'a> Visitor<'a> for CaseEmptyVisitor {
            type Value = CaseEmpty;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = CaseEmpty { type_, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(CaseEmptyVisitor)
    }
}
