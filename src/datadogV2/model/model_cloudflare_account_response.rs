...
impl<'de> Deserialize<'de> for CloudflareAccountResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct CloudflareAccountResponseVisitor;
        impl<'a> Visitor<'a> for CloudflareAccountResponseVisitor {
            type Value = CloudflareAccountResponse;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::CloudflareAccountResponseData> = None;
                ...
                let content = CloudflareAccountResponse { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(CloudflareAccountResponseVisitor)
    }
}
