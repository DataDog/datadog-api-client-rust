...
impl<'de> Deserialize<'de> for GCPSTSServiceAccountUpdateRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct GCPSTSServiceAccountUpdateRequestVisitor;
        impl<'a> Visitor<'a> for GCPSTSServiceAccountUpdateRequestVisitor {
            type Value = GCPSTSServiceAccountUpdateRequest;
            ...
            {
                let mut data: Option<
                    crate::datadogV2::model::GCPSTSServiceAccountUpdateRequestData,
                > = None;
                ...
                let content = GCPSTSServiceAccountUpdateRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(GCPSTSServiceAccountUpdateRequestVisitor)
    }
}
