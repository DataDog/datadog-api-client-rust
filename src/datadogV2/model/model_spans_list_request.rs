...
impl<'de> Deserialize<'de> for SpansListRequest {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct SpansListRequestVisitor;
        impl<'a> Visitor<'a> for SpansListRequestVisitor {
            type Value = SpansListRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::SpansListRequestData> = None;
                ...
                let content = SpansListRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(SpansListRequestVisitor)
    }
}
