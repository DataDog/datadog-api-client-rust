...
impl<'de> Deserialize<'de> for FastlyServiceAttributes {
    ...
    where
        D: Deserializer<'de>,
    {
        struct FastlyServiceAttributesVisitor;
        impl<'a> Visitor<'a> for FastlyServiceAttributesVisitor {
            type Value = FastlyServiceAttributes;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = FastlyServiceAttributes { tags, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(FastlyServiceAttributesVisitor)
    }
}
