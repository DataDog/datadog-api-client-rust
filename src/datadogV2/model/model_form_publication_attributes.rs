...
pub struct FormPublicationAttributes {
...
impl FormPublicationAttributes {
    pub fn new(
...
impl<'de> Deserialize<'de> for FormPublicationAttributes {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct FormPublicationAttributesVisitor;
        impl<'a> Visitor<'a> for FormPublicationAttributesVisitor {
            type Value = FormPublicationAttributes;
                ...
                M: MapAccess<'a>,
            {
                ...
                let form_version =
                    form_version.ok_or_else(|| M::Error::missing_field("form_version"))?;
                let modified_at =
                    modified_at.ok_or_else(|| M::Error::missing_field("modified_at"))?;
                let org_id = org_id.ok_or_else(|| M::Error::missing_field("org_id"))?;
                let publish_seq =
                    publish_seq.ok_or_else(|| M::Error::missing_field("publish_seq"))?;
                ...
                let content = FormPublicationAttributes {
                    created_at,
                    form_id,
                    form_version,
                    id,
        ...
        deserializer.deserialize_any(FormPublicationAttributesVisitor)
    }
}
