...
impl<'de> Deserialize<'de> for AuthNMappingCreateRequest {
    ...
    where
        D: Deserializer<'de>,
    {
        struct AuthNMappingCreateRequestVisitor;
        impl<'a> Visitor<'a> for AuthNMappingCreateRequestVisitor {
            type Value = AuthNMappingCreateRequest;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut data: Option<crate::datadogV2::model::AuthNMappingCreateData> = None;
                ...
                let content = AuthNMappingCreateRequest { data, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(AuthNMappingCreateRequestVisitor)
    }
}
