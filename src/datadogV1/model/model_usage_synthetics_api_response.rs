...
impl<'de> Deserialize<'de> for UsageSyntheticsAPIResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct UsageSyntheticsAPIResponseVisitor;
        impl<'a> Visitor<'a> for UsageSyntheticsAPIResponseVisitor {
            type Value = UsageSyntheticsAPIResponse;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut usage: Option<Vec<crate::datadogV1::model::UsageSyntheticsAPIHour>> = None;
                ...
                let content = UsageSyntheticsAPIResponse { usage, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(UsageSyntheticsAPIResponseVisitor)
    }
}
