...
impl<'de> Deserialize<'de> for SyntheticsPrivateLocationSecretsAuthentication {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct SyntheticsPrivateLocationSecretsAuthenticationVisitor;
        impl<'a> Visitor<'a> for SyntheticsPrivateLocationSecretsAuthenticationVisitor {
            type Value = SyntheticsPrivateLocationSecretsAuthentication;
                ...
                M: MapAccess<'a>,
            {
                ...
                let content = SyntheticsPrivateLocationSecretsAuthentication { id, key, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(SyntheticsPrivateLocationSecretsAuthenticationVisitor)
    }
}
