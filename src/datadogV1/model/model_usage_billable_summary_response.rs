...
impl<'de> Deserialize<'de> for UsageBillableSummaryResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct UsageBillableSummaryResponseVisitor;
        impl<'a> Visitor<'a> for UsageBillableSummaryResponseVisitor {
            type Value = UsageBillableSummaryResponse;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut usage: Option<Vec<crate::datadogV1::model::UsageBillableSummaryHour>> =
                    None;
                ...
                let content = UsageBillableSummaryResponse { usage, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(UsageBillableSummaryResponseVisitor)
    }
}
