...
impl<'de> Deserialize<'de> for UsageNetworkHostsResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct UsageNetworkHostsResponseVisitor;
        impl<'a> Visitor<'a> for UsageNetworkHostsResponseVisitor {
            type Value = UsageNetworkHostsResponse;
            ...
            where
                M: MapAccess<'a>,
            {
                let mut usage: Option<Vec<crate::datadogV1::model::UsageNetworkHostsHour>> = None;
                ...
                let content = UsageNetworkHostsResponse { usage, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(UsageNetworkHostsResponseVisitor)
    }
}
