// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.

use serde::de::{Error, MapAccess, Visitor};
use serde::{Deserialize, Deserializer, Serialize};
use serde_with::skip_serializing_none;
use std::fmt::{self, Formatter};

/// Represent validation endpoint responses.
#[non_exhaustive]
#[skip_serializing_none]
#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct AuthenticationValidationResponse {
    /// Return `true` if the authentication response is valid.
    #[serde(rename = "valid")]
    pub valid: Option<bool>,
    #[serde(skip)]
    #[serde(default)]
    pub(crate) _unparsed: bool,
}

impl AuthenticationValidationResponse {
    pub fn new() -> AuthenticationValidationResponse {
        AuthenticationValidationResponse {
            valid: None,
            _unparsed: false,
        }
    }

    pub fn valid(mut self, value: bool) -> Self {
        self.valid = Some(value);
        self
    }
}

impl Default for AuthenticationValidationResponse {
    fn default() -> Self {
        Self::new()
    }
}

impl<'de> Deserialize<'de> for AuthenticationValidationResponse {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct AuthenticationValidationResponseVisitor;
        impl<'a> Visitor<'a> for AuthenticationValidationResponseVisitor {
            type Value = AuthenticationValidationResponse;

            fn expecting(&self, f: &mut Formatter<'_>) -> fmt::Result {
                f.write_str("a mapping")
            }

            fn visit_map<M>(self, mut map: M) -> Result<Self::Value, M::Error>
            where
                M: MapAccess<'a>,
            {
                let mut valid: Option<bool> = None;
                let mut _unparsed = false;

                while let Some((k, v)) = map.next_entry::<String, serde_json::Value>()? {
                    match k.as_str() {
                        "valid" => {
                            if v.is_null() {
                                continue;
                            }
                            valid = Some(serde_json::from_value(v).map_err(M::Error::custom)?);
                        }
                        &_ => {}
                    }
                }

                let content = AuthenticationValidationResponse { valid, _unparsed };

                Ok(content)
            }
        }

        deserializer.deserialize_any(AuthenticationValidationResponseVisitor)
    }
}
