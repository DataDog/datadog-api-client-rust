...
use datadog_api_client::datadogV2::model::DeploymentRuleOptionsMonitorId;
use datadog_api_client::datadogV2::model::DeploymentRuleOptionsMonitorIds;
...
async fn main() {
    ...
    let body = CreateDeploymentRuleParams::new().data(CreateDeploymentRuleParamsData::new(
        CreateDeploymentRuleParamsDataAttributes::new(
            "Specific monitor deployment rule".to_string(),
            DeploymentRulesOptions::DeploymentRuleOptionsMonitorIds(Box::new(
                DeploymentRuleOptionsMonitorIds::new(vec![DeploymentRuleOptionsMonitorId::new(
                    vec![],
                    "7".to_string(),
                )]),
            )),
