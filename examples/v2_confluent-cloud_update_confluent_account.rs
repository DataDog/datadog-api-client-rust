...
async fn main() {
    ...
    let confluent_account_data_attributes_api_key = std::env::var("CONFLUENT_ACCOUNT_DATA_ATTRIBUTES_API_KEY").unwrap();
    let confluent_account_data_id = std::env::var("CONFLUENT_ACCOUNT_DATA_ID").unwrap();
    let body =
        ConfluentAccountUpdateRequest::new(
            ConfluentAccountUpdateRequestData::new(
                ConfluentAccountUpdateRequestAttributes::new(
                    confluent_account_data_attributes_api_key,
    ...
    let resp = api.update_confluent_account(confluent_account_data_id, body).await;
