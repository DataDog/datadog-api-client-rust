...
async fn main() {
    ...
    configuration.set_unstable_operation_enabled("v2.GetTwilioIntegrationAccount", true);
    let api = TwilioIntegrationAccountsAPI::with_config(configuration);
    let resp = api
        .get_twilio_integration_account(TwilioInterfaceType::TWILIO, "account_id".to_string())
        .await;
