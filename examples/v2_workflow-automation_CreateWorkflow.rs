// Create Workflow returns "Successfully created a workflow." response
...
async fn main() {
    let body = CreateWorkflowRequest::new(WorkflowData::new(
        WorkflowDataAttributes::new(
            ...
            Spec::new()
                ...
                .triggers(vec![
                    Trigger::MonitorTriggerWrapper(Box::new(MonitorTriggerWrapper::new(
                        MonitorTrigger::new().rate_limit(
                            TriggerRateLimit::new().count(1).interval("1h".to_string()),
                        ),
                        vec!["Step1".to_string()],
                    ))),
                    Trigger::GithubWebhookTriggerWrapper(Box::new(
                        GithubWebhookTriggerWrapper::new(GithubWebhookTrigger::new(), vec![]),
                    )),
                ]),
        )
    ...
    configuration.set_unstable_operation_enabled("v2.CreateWorkflow", true);
