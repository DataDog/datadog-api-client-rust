...
use datadog_api_client::datadogV2::model::TwilioAccountAttributes;
use datadog_api_client::datadogV2::model::TwilioAccountCreateData;
use datadog_api_client::datadogV2::model::TwilioAccountRequest;
...
#[tokio::main]
async fn main() {
    let body = TwilioAccountRequest::new(TwilioAccountCreateData::new(
        TwilioAccountAttributes::new(
        ...
        .settings(
            TwilioSettings::new("ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx".to_string()).censor_logs(true),
        ),
        IntegrationAccountType::INTEGRATION_ACCOUNT,
    ));
    let mut configuration = datadog::Configuration::new();
    configuration.set_unstable_operation_enabled("v2.CreateTwilioAccount", true);
    let api = TwilioIntegrationAccountsAPI::with_config(configuration);
    let resp = api.create_twilio_account(body).await;
