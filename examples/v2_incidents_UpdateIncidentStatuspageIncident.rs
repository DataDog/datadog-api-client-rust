...
async fn main() {
    ...
    configuration.set_unstable_operation_enabled("v2.UpdateIncidentStatuspageIncident", true);
    let api = IncidentsAPI::with_config(configuration);
    let resp = api
        .update_incident_statuspage_incident(
            "incident_id".to_string(),
            "page_id".to_string(),
            "statuspage_incident_id".to_string(),
            body,
        )
        .await;
