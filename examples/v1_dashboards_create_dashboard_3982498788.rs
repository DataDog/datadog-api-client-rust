...
#[tokio::main]
async fn main() {
    let body = Dashboard::new(DashboardLayoutType::ORDERED, "Example-Dashboard with timeseries widget".to_string(), vec![Widget::new(WidgetDefinition::TimeseriesWidgetDefinition(Box::new(TimeseriesWidgetDefinition::new(vec![TimeseriesWidgetRequest::new().display_type(WidgetDisplayType::BARS).on_right_yaxis(false).q("sum:trace.test.errors{env:prod,service:datadog-api-spec} by {resource_name}.as_count()".to_string()).style(WidgetRequestStyle::new().line_type(WidgetLineType::SOLID).line_width(WidgetLineWidth::NORMAL).palette("warm".to_string())),], TimeseriesWidgetDefinitionType::TIMESERIES, ))), ),], );
