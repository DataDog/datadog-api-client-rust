...
async fn main() {
    ...
    configuration.set_unstable_operation_enabled("v2.DeleteWebIntegrationAccount", true);
    let api = WebIntegrationsAPI::with_config(configuration);
    let resp = api
        .delete_web_integration_account("integration_name".to_string(), "account_id".to_string())
        .await;
