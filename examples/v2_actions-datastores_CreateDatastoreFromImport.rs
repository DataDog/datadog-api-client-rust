...
async fn main() {
    let body = CreateAppsDatastoreFromImportRequest::new().data(
        CreateAppsDatastoreFromImportRequestData::new(DatastoreDataType::DATASTORES).attributes(
            CreateAppsDatastoreFromImportRequestDataAttributes::new(
                "datastore-name".to_string(),
                "primaryKey".to_string(),
                vec![
                    BTreeMap::from([
                        ("primaryKey".to_string(), Value::from("key1")),
                        ("value".to_string(), Value::from("Newton")),
                    ]),
                    BTreeMap::from([
                        ("primaryKey".to_string(), Value::from("key2")),
                        ("value".to_string(), Value::from("Leibniz")),
                    ]),
                ],
            ),
        ),
