...
use datadog_api_client::datadogV2::api_threat_intelligence::SubmitStixThreatIntelOptionalParams;
...
async fn main() {
    ...
    configuration.set_unstable_operation_enabled("v2.SubmitStixThreatIntel", true);
    let api = ThreatIntelligenceAPI::with_config(configuration);
    let resp = api
        .submit_stix_threat_intel(
            "Acme-Inc".to_string(),
            body,
            SubmitStixThreatIntelOptionalParams::default(),
        )
        .await;
