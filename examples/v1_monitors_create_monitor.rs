...
async fn main() {
    ...
    let body = Monitor::new(r#"logs("service:foo AND type:error").index("main").rollup("count").by("source").last("5m") > 2"#.to_string(), MonitorType::LOG_ALERT, ).message("some message Notify: @hipchat-channel".to_string()).name("Example-Monitor".to_string()).priority(Some(3)).restricted_roles(Some(vec![role_data_id.clone(),])).tags(vec!["test:examplemonitor".to_string(),"env:ci".to_string(),]);
