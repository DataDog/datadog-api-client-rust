...
#[tokio::main]
async fn main() {
    let body = Dashboard::new(DashboardLayoutType::FREE, "Example-Dashboard".to_string(), vec![Widget::new(WidgetDefinition::ToplistWidgetDefinition(Box::new(ToplistWidgetDefinition::new(vec![ToplistWidgetRequest::new().formulas(vec![WidgetFormula::new("query1".to_string(), ),]).queries(vec![FormulaAndFunctionQueryDefinition::FormulaAndFunctionMetricQueryDefinition(Box::new(FormulaAndFunctionMetricQueryDefinition::new(FormulaAndFunctionMetricDataSource::METRICS, "query1".to_string(), "avg:system.cpu.user{*} by {service}".to_string(), ).aggregator(FormulaAndFunctionMetricAggregation::AVG))),]).response_format(FormulaAndFunctionResponseFormat::SCALAR).sort(WidgetSortBy::new().count(10).order_by(vec![WidgetSortOrderBy::WidgetGroupSort(Box::new(WidgetGroupSort::new("service".to_string(), WidgetSort::ASCENDING, GroupType::GROUP, ))),])),], ToplistWidgetDefinitionType::TOPLIST, ).style(ToplistWidgetStyle::new().display(ToplistWidgetDisplay::ToplistWidgetStacked(Box::new(ToplistWidgetStacked::new(ToplistWidgetStackedType::STACKED, ).legend(ToplistWidgetLegend::INLINE)))).palette("dog_classic".to_string()).scaling(ToplistWidgetScaling::RELATIVE)).time(WidgetTime::WidgetLegacyLiveSpan(Box::new(WidgetLegacyLiveSpan::new()))).title("".to_string()).title_align(WidgetTextAlign::LEFT).title_size("16".to_string()))), ).layout(WidgetLayout::new(15, 47, 0, 0, )),], ).description(Some("".to_string())).notify_list(Some(vec![])).template_variables(Some(vec![]));
