...
use datadog_api_client::datadogV2::model::RecommendedEntityWithSchema;

#[tokio::main]
async fn main() {
    let body = vec![RecommendedEntityWithSchema::new(
        "123abc456def".to_string(),
        EntityV3::EntityV3Service(Box::new(
            EntityV3Service::new(
    ...
    configuration.set_unstable_operation_enabled("v2.AcceptRecommendedEntities", true);
    let api = SoftwareCatalogAPI::with_config(configuration);
    let resp = api.accept_recommended_entities(body).await;
